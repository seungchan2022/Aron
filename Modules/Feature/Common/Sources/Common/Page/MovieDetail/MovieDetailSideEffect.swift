...
extension MovieDetailSideEffect {
  ...
  var routeToKeyword: (MovieEntity.MovieDetail.MovieCard.KeywordItem) -> Void {
    { item in
      navigator.next(
        linkItem: .init(
          path: Link.Movie.Path.keyword.rawValue,
          items: item),
        isAnimated: true)
    }
  }
  ...
  var routeToReview: (MovieEntity.MovieDetail.MovieCard.Response) -> Void {
    { item in
      navigator.next(
        linkItem: .init(
          path: Link.Common.Path.review.rawValue,
          items: item),
        isAnimated: true)
    }
  }
  ...
  var routeToCastList: (MovieEntity.MovieDetail.MovieCard.Response) -> Void {
    { item in
      navigator.next(
        linkItem: .init(
          path: Link.Credit.Path.cast.rawValue,
  ...
  var routeToCrewList: (MovieEntity.MovieDetail.MovieCard.Response) -> Void {
    { item in
      navigator.next(
        linkItem: .init(
          path: Link.Credit.Path.crew.rawValue,
          items: item.serializedCredit()),
        isAnimated: true)
    }
  }
  ...
  var routeToSimilarMovieList: (MovieEntity.MovieDetail.MovieCard.Response) -> Void {
    { item in
      navigator.next(
        linkItem: .init(
          path: Link.Movie.Path.similar.rawValue,
  ...
  var routeToRecommendedMovieList: (MovieEntity.MovieDetail.MovieCard.Response) -> Void {
    { item in
      navigator.next(
        linkItem: .init(
          path: Link.Movie.Path.recommended.rawValue,
  ...
  var routeToOtherPoster: (MovieEntity.MovieDetail.MovieCard.Response) -> Void {
    { item in
      navigator.fullSheet(
        linkItem: .init(
          path: Link.Common.Path.otherPoster.rawValue,
