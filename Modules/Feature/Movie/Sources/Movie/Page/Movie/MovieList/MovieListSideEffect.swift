...
public struct MovieListSideEffect {
...
extension MovieListSideEffect {
  ...
  var routeToNowPlaying: () -> Void {
    {
      navigator.next(
        linkItem: .init(
          path: Link.Movie.Path.nowPlaying.rawValue,
          items: NowPlayingRouteItem(isNavigationBarLargeTitle: true)),
        isAnimated: true)
    }
  }
  ...
  var routeToUpcoming: () -> Void {
    {
      navigator.next(
        linkItem: .init(path: Link.Movie.Path.upcoming.rawValue),
        isAnimated: true)
    }
  }
  ...
  var routeToTrending: () -> Void {
    {
      navigator.next(
        linkItem: .init(path: Link.Movie.Path.trending.rawValue),
        isAnimated: true)
    }
  }
  ...
  var routeToPopular: () -> Void {
    {
      navigator.next(
        linkItem: .init(path: Link.Movie.Path.popular.rawValue),
        isAnimated: true)
    }
  }
  ...
  var routeToTopRated: () -> Void {
    {
      navigator.next(
        linkItem: .init(path: Link.Movie.Path.topRated.rawValue),
        isAnimated: true)
    }
  }
}
