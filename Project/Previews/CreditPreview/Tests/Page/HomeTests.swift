...
final class HomeTests: XCTestCase {
  ...
  func test_binding_query_case2() async {
    ...
    newState.searchCreditItemList = ResponseMock().response.searchCredit.successValue.itemList
  ...
  func test_binding_query_case4() async {
    ...
    let CreditResponseMock: MovieEntity.Search.Credit.Response = ResponseMock().response.searchCredit.successValue
    ...
    newState.searchCreditItemList = CreditResponseMock.itemList
  ...
  @MainActor
  func test_searchCredit_success_case() async {
    let sut = SUT()

    let responseMock: MovieEntity.Search.Credit.Composite = .init(
      request: .init(query: "test"),
      response: ResponseMock().response.searchCredit.successValue)

    await sut.store.send(.set(\.query, "test")) { state in
      state.query = "test"
    }

    await sut.store.send(.searchCredit("test")) { state in
      state.fetchSearchCreditItem.isLoading = true
    }

    await sut.scheduler.advance()

    await sut.store.receive(\.fetchSearchCreditItem) { state in
      state.fetchSearchCreditItem.isLoading = false
      state.fetchSearchCreditItem.value = responseMock
      state.searchCreditItemList = responseMock.response.itemList
    }
  }

  @MainActor
  func test_searchCredit_failure_case() async {
    let sut = SUT()
  ...
  @MainActor
  func test_routeToSearchCredit_case() async {
    let sut = SUT()

    let pick: MovieEntity.Search.Credit.Item = ResponseMock().response.searchCredit.successValue.itemList.first!

    await sut.store.send(.routeToSearchCredit(pick))
  }
}
