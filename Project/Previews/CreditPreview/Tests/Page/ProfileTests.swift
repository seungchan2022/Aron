...
final class ProfileTests: XCTestCase {
  ...
  func test_getItem_success_case() async {
    ...
    let requestMock: MovieEntity.Credit.Info.Request = .init(CreditID: 2)
    let responseMock: MovieEntity.Credit.Info.Response = ResponseMock().response.info.successValue
  ...
  func test_getItem_failure_case() async {
    ...
    sut.container.CreditUseCaseStub.type = .failure(.invalidTypeCasting)
  ...
  func test_getProfileImage_success_case() async {
    ...
    let requestMock: MovieEntity.Credit.Image.Request = .init(CreditID: 2)
    let responseMock: MovieEntity.Credit.Image.Response = ResponseMock().response.image.successValue
  ...
  func test_getMovieCreditItem_success_case() async {
    ...
    let requestMock: MovieEntity.Credit.MovieCredit.Request = .init(CreditID: 2)
    let responseMock: MovieEntity.Credit.MovieCredit.Response = ResponseMock().response.movieCredit.successValue
  ...
  func test_routeToCastDetail_case() async {
    ...
    let pick: MovieEntity.Credit.MovieCredit.CastItem = (ResponseMock().response.movieCredit.successValue.castItemList.first)!
  ...
  func test_routeToCrewDetail_case() async {
    ...
    let pick: MovieEntity.Credit.MovieCredit.CrewItem = (ResponseMock().response.movieCredit.successValue.crewItemList.first)!
...
extension ProfileTests {
  struct SUT {
    ...
    init(state: ProfileReducer.State = .init(
      item: .init(CreditID: 2),
      profileImageItem: .init(CreditID: 2),
      movieCreditItem: .init(CreditID: 2)))
    {
  ...
  struct ResponseMock {
    let response: CreditUseCaseStub.Response = .init()
    init() { }
  }
}
