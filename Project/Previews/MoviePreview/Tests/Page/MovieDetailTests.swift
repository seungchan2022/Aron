...
final class MovieDetailTests: XCTestCase {
  ...
  @MainActor
  func test_getMovie_success_case() async {
    let sut = SUT()

    let requestMock: MovieEntity.MovieDetail.Movie.Request = .init(movieID: 1)
    let responseMock: MovieEntity.MovieDetail.Movie.Response = ResponseMock().response.Movie.successValue

    await sut.store.send(.getMovie(requestMock)) { state in
      state.fetchMovieItem.isLoading = true
    }

    await sut.scheduler.advance()

    await sut.store.receive(\.fetchMovieItem) { state in
      state.fetchMovieItem.isLoading = false
      state.fetchMovieItem.value = responseMock
    }
  }

  @MainActor
  func test_getMovie_failure_case() async {
    let sut = SUT()
  ...
  func test_routeToCastItem_case() async {
    ...
    let pick: MovieEntity.MovieDetail.Movie.CastItem = ResponseMock().response.Movie.successValue.castItemList.first!
  ...
  func test_routeToCrewItem_case() async {
    ...
    let pick: MovieEntity.MovieDetail.Movie.CrewItem = ResponseMock().response.Movie.successValue.crewItemList.first!
