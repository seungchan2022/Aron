...
final class KeywordTets: XCTestCase {
  ...
  func test_getItem_success_case() async {
    ...
    let mock: MovieEntity.MovieDetail.MovieCard.KeywordItem = .init(id: 2, name: "test")

    let responseMock: MovieEntity.Discover.Keyword.Response = ResponseMock().response.keyword.successValue
...
extension KeywordTets {
  struct SUT {

    // MARK: Lifecycle

    init(state: KeywordReducer.State = .init(item: .init(id: 2, name: "test"))) {
    ...
    let store: TestStore<KeywordReducer.State, KeywordReducer.Action>
  }
